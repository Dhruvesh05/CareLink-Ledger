import { env } from "../../config/env";

export type IpfsProtocol = "http" | "https";

export type IpfsProvider = "kubo" | "pinata";

export interface IIpfsConfig {
	readonly host: string;
	readonly port: number;
	readonly protocol: IpfsProtocol;
	readonly timeout: number;
	readonly provider: IpfsProvider;
	readonly apiUrl: string;
	readonly gatewayUrl: string;
	readonly pinningEndpoint?: string;
}

const DEFAULT_IPFS_HOST = "127.0.0.1";
const DEFAULT_IPFS_PORT = 5001;
const DEFAULT_IPFS_PROTOCOL: IpfsProtocol = "http";
const DEFAULT_IPFS_TIMEOUT = 30_000;

function normalizeProtocol(value: string | undefined): IpfsProtocol {
	return value === "https" ? "https" : "http";
}

function buildApiUrl(
	protocol: IpfsProtocol,
	host: string,
	port: number
): string {

	return `${protocol}://${host}:${port}/api/v0`;

}

function buildGatewayUrl(
	protocol: IpfsProtocol,
	host: string,
	port: number
): string {

	return `${protocol}://${host}:${port}/ipfs`;

}

const ipfsHost = process.env.IPFS_HOST || DEFAULT_IPFS_HOST;
const ipfsPort = Number(process.env.IPFS_PORT) || DEFAULT_IPFS_PORT;
const ipfsProtocol = normalizeProtocol(process.env.IPFS_PROTOCOL);

const provider: IpfsProvider = env.PINATA_JWT
	? "pinata"
	: "kubo";

export const ipfsConfig: IIpfsConfig = {
	host: ipfsHost,
	port: ipfsPort,
	protocol: ipfsProtocol,
	timeout: Number(process.env.IPFS_TIMEOUT_MS) || DEFAULT_IPFS_TIMEOUT,
	provider,
	apiUrl: buildApiUrl(ipfsProtocol, ipfsHost, ipfsPort),
	gatewayUrl: buildGatewayUrl(ipfsProtocol, ipfsHost, ipfsPort),
	pinningEndpoint: env.PINATA_JWT
		? "https://api.pinata.cloud"
		: undefined
};

export function getIpfsApiUrl(): string {
	return ipfsConfig.apiUrl;
}

export function getIpfsGatewayUrl(): string {
	return ipfsConfig.gatewayUrl;
}
